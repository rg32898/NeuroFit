...
- **Billing cron**: `POST /api/admin/cron/billing` (same `x-cron-secret` model as `/cron/daily`) runs FR-6.3 trial-end reminders (48h window, idempotent via `trial_reminder_sent_at`) + FR-6.4 receipt notifications (5-minute lookback). Notifications go to a `notifications` queue table; transport (email/push) is wired in Prompt 10.
